...
class Account:
    ...
    def borrow(self,amount): 
        ...
        if (len(self.deposits)) <10:
            return f"Can't be lended this {amount} you've deposited less than 10 times." 
        elif(amount <100):
            return"You can't borrow  less than 100" 
        elif amount > (sum//3):
            return "You've exceeded the loan limit"
        elif self.balance==0:
            return "You can't borrow.Your account balance is 0"   
        elif self.loan_balance>0:
            return f"You have an outstanding loan of {self.loan_balance}.You can't borrow this {amount}"    
        else:
            self.loan_balance+=(amount+(amount*0.03))
            return f"Your loan is {amount} and an interest of{amount*0.03} and total loan amount is {self.loan_balance}"   
            
    def loan_repayment(self,amount):
        overpayment=amount-self.loan_balance
        loan_payment=amount-overpayment
        if amount<0:
            return f"Repayment amount has to be more than 0"
            ...
            return f"You have an overpayment of {overpayment}"
    
    def transfer(self, amount, acc2):
        if amount > self.balance:
            return "You don't have sufficient amount of money to transfer this amount."
        elif amount<=0:
            return "Put a valid amount of money."
        else:    
            self.balance-=amount
            acc2.balance+=amount
            return f"{self.account_name} you've sent {amount} to {acc2.account_name}"    
